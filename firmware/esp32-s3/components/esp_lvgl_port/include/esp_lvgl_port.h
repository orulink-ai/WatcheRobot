...
typedef struct {
    ...
    int16_t sensitivity;           /*!< Touch sensitivity (0 - 255, for filtering weak signals) */
 ...
 * @return
 *      - ESP_OK                    on success
 *      - ESP_ERR_NOT_SUPPORTED     if event is BUTTON_PRESS_DOWN or BUTTON_PRESS_UP (reserved for LVGL)
 */
...
/**
 * @brief Draw bitmap through the shared LCD panel path and wait for transfer completion.
 *
 * This serializes low-level LCD writes with LVGL flushes so external drawing
 * paths do not overrun the SPI transaction queue or reuse DMA buffers while a
 * transfer is still active.
 *
 * @param panel_handle LCD panel handle
 * @param x_start Start index on x-axis
 * @param y_start Start index on y-axis
 * @param x_end End index on x-axis
 * @param y_end End index on y-axis
 * @param color_data RGB color data
 * @param timeout_ms Timeout in [ms]. 0 will block indefinitely.
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_TIMEOUT on timeout
 *      - ESP_ERR_INVALID_STATE if the LVGL port is not initialized
 *      - Other errors returned by esp_lcd_panel_draw_bitmap
 */
