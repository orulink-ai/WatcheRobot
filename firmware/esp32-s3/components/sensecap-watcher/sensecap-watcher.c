...
esp_io_expander_handle_t bsp_io_expander_init() {
    ...
    ret |= esp_io_expander_new_i2c_pca95xx_16bit_ex(BSP_GENERAL_I2C_NUM, ESP_IO_EXPANDER_I2C_PCA9535_ADDRESS_001,
...
inline static esp_err_t bsp_rtc_reg_write(uint8_t reg, uint8_t *val, size_t len) {
    ...
    return i2c_master_write_to_device(BSP_GENERAL_I2C_NUM, DRV_PCF8563_I2C_ADDR, data, len + 1,
                                      DRV_PCF8563_TIMEOUT_MS / portTICK_PERIOD_MS);
}

inline static esp_err_t bsp_rtc_reg_read(uint8_t reg, uint8_t *val, size_t len) {
    return i2c_master_write_read_device(BSP_GENERAL_I2C_NUM, DRV_PCF8563_I2C_ADDR, &reg, 1, val, len,
                                        DRV_PCF8563_TIMEOUT_MS / portTICK_PERIOD_MS);
}
