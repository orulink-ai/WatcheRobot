...
typedef struct {
    ...
    uint8_t low_battery_recover_percent;
...
typedef enum {
    POWER_MONITOR_BEHAVIOR_RECHARGE = 0,
    POWER_MONITOR_BEHAVIOR_STANDBY_AFTER_UNPLUG,
} power_monitor_behavior_t;

typedef bool (*power_monitor_behavior_gate_cb_t)(power_monitor_behavior_t behavior,
                                                 const power_monitor_event_t *event,
                                                 void *user_ctx);
