...
static power_monitor_logic_state_t make_state(void) {
    ...
    const power_monitor_logic_config_t config = {
        ...
        .low_battery_percent = 15u,
        .low_battery_recover_percent = 18u,
    };
...
static void test_low_battery_only_fires_without_vbus_and_uses_hysteresis(void) {
    power_monitor_logic_state_t state = make_state();
    power_monitor_event_t event;

    assert(update_sample(&state, true, true, false, 15u, &event) == ESP_OK);
    assert((event.flags & POWER_MONITOR_EVENT_LOW_BATTERY) == 0u);

    assert(update_sample(&state, false, false, false, 15u, &event) == ESP_OK);
    assert(update_sample(&state, false, false, false, 15u, &event) == ESP_OK);
    assert((event.flags & POWER_MONITOR_EVENT_LOW_BATTERY) != 0u);

    assert(update_sample(&state, false, false, false, 16u, &event) == ESP_OK);
    assert((event.flags & POWER_MONITOR_EVENT_LOW_BATTERY) == 0u);
    assert(state.low_battery_active);

    assert(update_sample(&state, false, false, false, 18u, &event) == ESP_OK);
    assert(!state.low_battery_active);
...
int main(void) {
    ...
    test_low_battery_only_fires_without_vbus_and_uses_hysteresis();
