...
static inline bool xmodem_verify_packet(const xmodem_packet_t packet, uint8_t expected_packet_id) {
    ...
    if (packet.preamble == XSOH && packet.id == expected_packet_id && packet.id_complement == 0xFF - packet.id &&
...
static xmodem_state_t xmodem_process(sscma_client_flasher_we2_uart_t *flasher) {
    ...
    case WRITE_BLOCK: {
        ...
        if (flasher->cur_packet.id != flasher->cur_packet_id || flasher->cur_packet_id == 1) {
            ...
            flasher->xfer_size = flasher->tx_buffer.len - flasher->tx_buffer.pos > XMODEM_BLOCK_SIZE
                                     ? XMODEM_BLOCK_SIZE
                                     : flasher->tx_buffer.len - flasher->tx_buffer.pos;
...
esp_err_t sscma_client_new_flasher_we2_uart(const sscma_client_io_handle_t io,
    ...
    if (config->flags.reset_use_expander) {
        ...
        ESP_GOTO_ON_ERROR(esp_io_expander_set_dir(flasher_we2->io_expander, config->reset_gpio_num, IO_EXPANDER_OUTPUT),
...
static esp_err_t sscma_client_flasher_we2_start(sscma_client_flasher_handle_t flasher, size_t offset) {
    ...
    if (flasher_we2->reset_gpio_num >= 0) {
        ...
        ESP_GOTO_ON_ERROR(sscma_client_io_write(flasher_we2->io, CMD_PREFIX CMD_AT_RESET CMD_SUFFIX,
                                                sizeof(CMD_PREFIX CMD_AT_RESET CMD_SUFFIX) - 1),
                          err, TAG, "reset sscma failed");
...
err:
    if (ret != ESP_OK) {
        if (flasher_we2->reset_gpio_num >= 0) {
            if (flasher_we2->io_expander) {
                esp_io_expander_set_level(flasher_we2->io_expander, flasher_we2->reset_gpio_num,
                                          flasher_we2->reset_level);
                vTaskDelay(50 / portTICK_PERIOD_MS);
                esp_io_expander_set_level(flasher_we2->io_expander, flasher_we2->reset_gpio_num,
                                          !flasher_we2->reset_level);
                vTaskDelay(50);
