...
6. 进入业务阶段后收发 JSON 文本帧，以及 `WSPK` 二进制音频/视频/图片/App.Center 应用包帧。
...
- `MCU_FRAME_CLASS_SENSOR / MCU_SENSOR_MSG_TOUCH_EVENT`
  ...
  - 按下事件只在行为系统空闲时触发，不抢占 TTS、工具调用、苏醒等前台行为
  - 固件在 `fondle_love` 与 `fondle_anger` 中随机选择一个可用的一次性表情；首选资源不可用时回退到另一个
  - 触摸表情结束后回到无文字、无声音的本地待机，并从该次交互重新计算 60 秒入睡时限
...
- 当前主协议下，设备需支持的下行消息类型：
  ...
  - `xfer.ota.handshake`
  - `xfer.ota.checksum`
  - `app.package.transfer.begin`
  - `app.package.transfer.commit`
...
- 当前主协议下，设备上行消息类型：
  ...
  - `evt.ota.progress`
  - `xfer.ota.handshake`
  - `evt.app.package.status`
  - `evt.app.package.list`
- 当前固件保留的兼容扩展：
  - `evt.camera.state`
...
- `evt.ai.status` 资源字段冻结：
  - `data.image_name`: 可选
  - `data.action_file`: 可选
  - `data.sound_file`: 可选；字段省略或显式传入空字符串时均继承映射状态的默认音效，非空值覆盖默认音效；当前协议不提供显式静音语义
  - 固件收到后优先按 `action_file -> status -> fallback` 顺序解析状态
  - AI 交互链路不显示 `message`、ASR、回复或过程文字；任务域成功终态只释放前台租约，不播放成功表情
- 文本类字段上限冻结：
...
- frame type 冻结：
  ...
  - `3 = image`
  - `4 = ota`
  - `5 = app_package`，用于 App.Center 应用包分片
- flags 冻结：
  - `FIRST=0x01`
  - `LAST=0x02`
  - `KEYFRAME=0x04`
...
### FZ-06.5 App.Center 应用包传输协议
- App.Center 应用包传输不是系统 OTA；它只安装 App.Center 可下载应用包。
- 桌面端先发送 `app.package.transfer.begin`，server 锁定目标设备，设备侧准备临时包。
- begin 成功后，桌面端通过 watcher-server 转发 `frame_type=5` 的 `WSPK` app_package 分片。
- 分片发送完成且设备回报接收完成后，桌面端发送 `app.package.transfer.commit`，设备侧执行 SHA-256、manifest、权限、兼容性、版本和签名校验。
- 分片失败、commit 前取消或需要清理半包时，桌面端发送 `app.package.transfer.abort`，设备侧丢弃临时包并回到可重新安装状态。
- 设备侧通过 `evt.app.package.status` 回报安装、打开、卸载和失败状态，通过 `evt.app.package.list` 回报已安装 App.Center app 快照。
- `app.package.open` / `app.package.uninstall` / `app.package.list` 必须只作用于 App.Center 可下载应用，不用于 Launcher 本地固定功能。
...
6. AI 状态：`evt.ai.status` 的 `status / message / image_name / action_file / sound_file` 能被固件解析并落到本地行为状态资源；当 `sound_file` 非空时，固件会按该资源名触发本地 SFX；字段省略或显式传入空字符串时，无论来自 BLE 还是 WebSocket/云端，均继承状态默认音效。当前协议不提供显式静音语义。
